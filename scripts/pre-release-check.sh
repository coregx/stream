#!/usr/bin/env bash
# Pre-Release Validation Script for Stream Library (SSE + WebSocket)
# This script runs all quality checks before creating a release
# EXACTLY matches CI checks + additional validations
# Based on MATLAB File Reader pre-release-check.sh
...
echo ""
echo "================================================"
echo "  Stream Library (SSE + WebSocket) - Pre-Release Check"
echo "================================================"
echo ""
...
go mod tidy
# Only check go.sum if it exists (zero-dependency projects won't have it)
if [ -f "go.sum" ]; then
    ...
    # No go.sum means zero dependencies - just check go.mod
    if git diff --quiet go.mod 2>/dev/null; then
        log_success "go.mod is tidy (zero dependencies)"
    else
        log_warning "go.mod needs tidying (run 'go mod tidy')"
        git diff go.mod
        WARNINGS=$((WARNINGS + 1))
...
if command -v gcc &> /dev/null || command -v clang &> /dev/null; then
    ...
    if [ -n "$WSL_DISTRO" ]; then
        ...
        TEST_CMD="wsl -d \"$WSL_DISTRO\" bash -c \"cd \\\"$WSL_PATH\\\" && GOEXPERIMENT=jsonv2 go test -race -ldflags '-linkmode=external' ./... 2>&1\""
    else
...
if [ $USE_WSL -eq 1 ]; then
    ...
    TEST_OUTPUT=$(wsl -d "$WSL_DISTRO" bash -c "cd $WSL_PATH && GOEXPERIMENT=jsonv2 timeout 180 stdbuf -oL -eL go test -race -ldflags '-linkmode=external' ./... 2>&1" || true)
...
if echo "$TEST_OUTPUT" | grep -q "hole in findfunctab\|build failed.*race"; then
    ...
    if [ $USE_WSL -eq 1 ]; then
        TEST_OUTPUT=$(wsl -d "$WSL_DISTRO" bash -c "cd \"$WSL_PATH\" && GOEXPERIMENT=jsonv2 go test ./... 2>&1")
    else
...
# Check coverage for stream packages (sse/ and websocket/)
if [ -d "sse" ]; then
    SSE_COVERAGE=$(go test -cover ./sse/... 2>&1 | grep "coverage:" | tail -1 | awk '{print $5}' | sed 's/%//')
    if [ -n "$SSE_COVERAGE" ]; then
        echo "  • sse/ coverage: ${SSE_COVERAGE}%"
        if awk -v cov="$SSE_COVERAGE" 'BEGIN {exit !(cov >= 80.0)}'; then
            log_success "SSE coverage meets requirement (>80%)"
        else
            log_warning "SSE coverage below 80% (${SSE_COVERAGE}%)"
            WARNINGS=$((WARNINGS + 1))
        fi
    else
        log_info "No tests in sse/ yet (early development)"
    fi
fi

if [ -d "websocket" ]; then
    WS_COVERAGE=$(go test -cover ./websocket/... 2>&1 | grep "coverage:" | tail -1 | awk '{print $5}' | sed 's/%//')
    if [ -n "$WS_COVERAGE" ]; then
        echo "  • websocket/ coverage: ${WS_COVERAGE}%"
        if awk -v cov="$WS_COVERAGE" 'BEGIN {exit !(cov >= 80.0)}'; then
            log_success "WebSocket coverage meets requirement (>80%)"
        else
            log_warning "WebSocket coverage below 80% (${WS_COVERAGE}%)"
            WARNINGS=$((WARNINGS + 1))
        fi
    else
        log_info "No tests in websocket/ yet (early development)"
    fi
fi
...
if [ -n "$OVERALL_COVERAGE" ]; then
    ...
    if awk -v cov="$OVERALL_COVERAGE" 'BEGIN {exit !(cov >= 80.0)}'; then
        log_success "Overall coverage meets requirement (>80%)"
    else
        log_warning "Overall coverage below 80% (${OVERALL_COVERAGE}%)"
        WARNINGS=$((WARNINGS + 1))
    fi
else
...
# 9. Check minimal dependencies policy
log_info "Checking dependencies policy..."
if [ -f "go.mod" ]; then
    # Count non-stdlib dependencies (excluding indirect and plugins)
    CORE_DEPS=$(grep -E "^\s+github.com|^\s+golang.org" go.mod | grep -v "// indirect" | grep -v "plugins/" | wc -l)

    # Expected dependencies: ZERO (stream uses only stdlib)
    EXPECTED_DEPS=0

    if [ "$CORE_DEPS" -eq 0 ]; then
        log_success "Zero external dependencies policy met ✅"
        log_info "  Stream library uses ONLY stdlib (pure Go)"
    else
        log_warning "Unexpected dependencies found: $CORE_DEPS (expected: 0)"
        log_info "  Stream library should have ZERO external dependencies!"
